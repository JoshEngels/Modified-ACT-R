...
;;; 2017.03.20 Dan [1.1]
;;;             : * Fixed some potential problems with the parameter settings.
;;;             :   Instead of trying to force the parameters to be what the
;;;             :   module wants when they are changed just print a warning and
;;;             :   then disable the mechanism.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;
;;; General Docs:
;;; 
         ...
         (case (car param)
           (:ol
            (when (and (cdr param)
              ...
              (print-warning "Turning on :ol disables the spacing effect.  :eblse has been turned off.")
              (no-output (sgp :eblse nil))))
           (:bll 
            (when (spacing-effect-enabled module)
              (cond ((and (numberp (cdr param)) (not (= (cdr param) 91923.12)))
                     (print-warning "Changing :bll has no effect when :eblse is enabled")
                     ;; don't try and force it -- just let the warnings continue
                     ;(no-output (sgp :bll 91923.12))
                     )
                    ((not (numberp (cdr param)))
                     (print-warning "Turning off :bll disables the spacing effect.  :eblse has been turned off.")
                     (no-output (sgp :eblse nil))))))
           (:bl-hook 
            (when (and (spacing-effect-enabled module)
                       (not (equal (cdr param) 'compute-spacing-effect-activation)))
              (print-warning "Changing the :bll-hook disables the spacing effect.  :eblse has been turned off.")
              (no-output (sgp :eblse nil))))
           (:eblse (setf (spacing-effect-enabled module) (cdr param))
                   (if (cdr param)
                       (no-output (sgp :ol nil :bll 91923.12 :bl-hook compute-spacing-effect-activation))
                     ;; when its turned off remove the hook function
                     (when (equal (no-output (car (sgp :bl-hook))) 'compute-spacing-effect-activation)
                       (sgp :bl-hook nil)))
                   (cdr param))
